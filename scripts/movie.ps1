...
## EVERY NATIVE CALL BELOW RUNS THROUGH THIS.
##
## Under `$ErrorActionPreference = 'Stop'` a native command's stderr becomes a
## TERMINATING ErrorRecord, thrown before `$LASTEXITCODE` is ever read - and
## redirection does not save it, because `*>` sends the text to the log while
## the ErrorRecord still terminates. Godot writes routine warnings to stderr
## ("4 ObjectDB instances were leaked at exit"), so the first filmed run of this
## game died at the Godot call with 480 frames already on disk and reported it
## as a NativeCommandError, which reads like the film failing.
##
## The exit code is what this script actually branches on, so the preference has
## to be relaxed for the duration of each call. Same fix `check.ps1` and
## `new-game.ps1` needed; still owed to the copy in `C:\dev\godot-template`.
