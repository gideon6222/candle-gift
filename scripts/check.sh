...
# `pipefail` as well as `-u`: `step` runs its command through `tr`, and without
# this `$?` is TR's status - always 0 - so the exit-code branch below was dead
# and only the text grep decided whether a step passed. A run that exited
# non-zero without printing a matching line went green.
set -u -o pipefail
...
step () {
	...
	if [ $code -ne 0 ] || echo "$out" | grep -qE "FAILED|SCRIPT ERROR|Parse Error|^ERROR:"; then
		echo "FAIL  $name"
