...
- [ ] **1.1 Everything sits on the ground.** Shadows on, tuned for a mobile GPU, with the
...
- [ ] **1.2 The runway has depth.** Distance fog whose colour is the sky's horizon end, so the
...
- [ ] **1.3 The batch is never clipped by the bottom of the frame.** In sheet cells 6-8 the
...
- [ ] **1.4 Wax and wax-coated candles read as wax.** A sheen that moves with the light, so a
      dipped candle is distinguishable from a painted one.
...
- [ ] **2.1 Value appears where it is earned.** World-space floating text at the candle that
...
- [ ] **2.2 A dip splashes, glitter sparkles, a hit bursts.** `assets/particles/` already has
      droplet, glitter and glow and only the ladle's pour uses any of them.
- [ ] **2.3 A hit is felt.** Camera kick and a colour flash on losing candles, and the frame
...
- [ ] **3.1 The finished batch stands on a dark navy podium** at the ruler, instead of being
      lit in place on the runway. Screenshot 5 shows it clearly.
- [ ] **3.2 Price tags carry their number.** Theirs read `5 $`, `154 $`; ours are blank.
- [ ] **3.3 Shop fronts are bought in the track**, with a green `+` per panel, rather than
      being scenery that opens a sheet.
...
- [ ] **4.1 The Android back button works.** `quit_on_go_back = false` AND
...
- [ ] **4.2 The version cannot drift.** A pure test asserting `Changelog.VERSION` equals
      `version/name` in **each** export preset and `RELEASES[0].version`.
- [ ] **4.3 The game can be filmed.** There is no `scripts/movie.ps1` and no `test/replays/`,
